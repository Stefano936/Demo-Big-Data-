...
def construir_curated():
    ...
    merged = merged.drop(columns=["titletype", "endyear"])

    # Generar tabla de géneros
    
    generos = merged.drop(columns=["primarytitle","originaltitle","isadult","startyear","runtimeminutes","averageRating","numVotes"])
    generos = generos.assign(genero=generos['genres'].str.split(',')).explode('genero').drop_duplicates().reset_index(drop=True)
    generos = generos[['tconst','genero']]
    generos.to_csv(CURATED_DIR / "generos_table.csv", index=False)
